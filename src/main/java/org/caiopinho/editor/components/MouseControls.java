...
public class MouseControls extends Component {
		...
		if (MouseListener.isButtonDown(GLFW_MOUSE_BUTTON_LEFT)) {
			...
			for (GameObject gameObject : Window.getScene().getGameObjects()) {
				...
				if (SquarePhysics.isPointInsideRectangle(
						...
						new Transform(gameObject.transform.position, getBoxSelectionScale(gameObject))
				)) {
	...
	@Override public void update(float deltaTime) {
		...
		if (this.holdingGameObject != null) {
			drawGameObjectSelectionCircle(this.holdingGameObject, SELECTION_COLOR);
			...
			if (this.moveGameObjectEnabled) {
				if (this.gridModeEnabled) {
					...
					this.holdingGameObject.transform.position.y = MouseListener.getOrthoY() - this.holdingGameObject.transform.scale.y / 2;
				}
			}
